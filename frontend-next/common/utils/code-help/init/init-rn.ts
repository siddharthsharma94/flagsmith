...
const res = (envId: string, data:codeHelpType, customFeature:string) =>  `import ${data.LIB_NAME} from ${data.NPM_RN_CLIENT}; //Add this line if you're using ${data.LIB_NAME} via npm
...
${data.LIB_NAME}.init({
    ...
    onChange: (oldFlags, params) => { // Occurs whenever flags are changed
        ...
        const ${data.FEATURE_NAME_ALT}Old = oldFlags["${customFeature || data.FEATURE_NAME_ALT}"] && oldFlags["${customFeature || data.FEATURE_NAME_ALT}"].value;
